...
ui <- navbarPage("Nature-positive climate risk transfer & financing instruments", id = "main",
...
subnat_shapes_all <- dplyr::bind_rows(
  usa_adm1 |> mutate(country = "United States of America"),
  china_adm1 |> mutate(country = "China"),
  mexico_adm1 |> mutate(country = "Mexico"),
  nicaragua_adm1 |> mutate(country = "Nicaragua"),
  philippines_adm1 |> mutate(country = "Philippines")
)
  ...
  output$inventorymap <- renderLeaflet({
    ...
    if (scale_choice == "subnational (province, district)") {
      ...
      inventory_subnat <- inventory_short %>%
        ...
        mutate(location = ifelse(country == "Nicaragua", 
                                 gsub(".*: ", "", location), location)) %>%
        separate_rows(location, sep = ";") %>%        # separate multiple locations
        mutate(location = trimws(location)) %>%       # remove whitespace
        # Map each location to correct GADM NAME_1
        mutate(location_subnational = case_when(
